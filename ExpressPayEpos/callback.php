...
require_once(dirname(__FILE__).'/ExpressPayEposView.php');
...
function callbackPayment($logs){
	...
	$data['status'] = 'payment';
	$data['epos_number'] = $settings['key_service'].'-'.$settings['key_epos'].'-'.$_REQUEST['ExpressPayAccountNumber'];
	
	//Вывод сообщения в шаблон
	$expressPayView = new ExpressPayEposView();
...
function callbackFail($logs){
	...
	$data['status'] = 'fail';
	$data['message'] = 'При выполнении запроса оплаты через E-POS произошла непредвиденная ошибка. Пожалуйста, повторите запрос позже или обратитесь в службу технической поддержки магазина.';
